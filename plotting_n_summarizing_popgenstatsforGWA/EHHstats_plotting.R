...
library(data.table)
obs<-fread("EHH_5kwindowed_allTSRobserved.txt")
...
obs_test<- obs[obs$win < -1 | obs$win > 1,]
exp_test<- exp[exp$win < -1 | exp$win > 1,]

#%>% filter(win < 11 & win > -11 ) 
obs_avg<-obs_test %>% group_by(chr,ps) %>% summarise(EHH1_mean=mean(ehh1),EHH2_mean=mean(ehh0),EHH1_SE=se(ehh1),EHH2_SE=se(ehh0))
exp_avg<- exp_test %>% group_by(chr,ps) %>% summarise(EHH1_mean=mean(ehh1),EHH2_mean=mean(ehh0),EHH1_SE=se(ehh1),EHH2_SE=se(ehh0))
x=nrow(exp_avg)
y=nrow(obs_avg)
...
quantile(perm_stats$mean_EHH1,probs = c(0.05, 0.95))
median(obs_avg$EHH1_mean)

quantile(perm_stats$mean_EHH2,probs = c(0.05, 0.95))
median(obs_avg$EHH2_mean)
